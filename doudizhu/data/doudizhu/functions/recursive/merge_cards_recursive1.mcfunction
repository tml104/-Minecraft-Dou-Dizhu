...
execute unless entity @e[tag=doudizhu,tag=Card,tag=!Onlyback,type=armor_stand,distance=..0.1,tag=!Selected] run summon area_effect_cloud ~ ~ ~ {Duration:2147483647,Tags:["MC_1","doudizhu"]}
execute unless entity @e[tag=doudizhu,tag=Card,tag=!Onlyback,type=armor_stand,distance=..0.1,tag=!Selected] positioned ^-0.01 ^ ^0.4 run function doudizhu:recursive/merge_cards_recursive2
